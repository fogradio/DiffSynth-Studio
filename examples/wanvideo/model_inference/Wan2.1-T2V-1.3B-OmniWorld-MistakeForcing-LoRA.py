"""Inference script: Wan2.1-T2V-1.3B with LoRA DiT (no copilot).

Loads the BASE DiT, fuses a LoRA checkpoint on top, then runs standard
pipeline inference.  Mirrors the MistakeForcing inference script but swaps
full-FT weights for base + LoRA.
"""
import argparse
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Wan2.1-T2V-1.3B inference with LoRA DiT weights",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--base_model_root", type=str,
        default="/mnt/workspace/common/models/Wan2.1-T2V-1.3B",
    )
    parser.add_argument(
        "--dit_weights", type=str, default="",
    ...
    parser.add_argument("--tag", type=str, default="omniworld_lora")
