"""Inference script: Wan2.1-T2V-1.3B with LoRA DiT + Video Copilot correction.

Same denoising loop as the full-FT copilot inference script, but loads the
*base* DiT and fuses a LoRA checkpoint on top before running inference.
"""
...
MISTAKE_FORCING_DIR = Path(__file__).resolve().parents[4]
...
# ---------------------------------------------------------------------------
# Copilot model loader (same as CopilotInference.py)
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Wan2.1-T2V-1.3B inference with LoRA DiT + Video Copilot",
    )
    ...
    p.add_argument("--dit_weights", type=str, default="",
                    help="Base DiT weights. Default: <base_model_root>/diffusion_pytorch_model.safetensors")
    p.add_argument("--lora_weights", type=str, required=True,
                    help="Path to LoRA checkpoint (.safetensors) to fuse into the base DiT.")
    ...
    p.add_argument("--tag", type=str, default="lora_copilot")
    p.add_argument("--limit", type=int, default=0)

    # ---- copilot ----
    p.add_argument("--copilot_variant", type=str, default="v2", choices=["v1", "v2"])
    p.add_argument("--copilot_ckpt", type=str, required=True,
    ...
    *,
    ...
    for progress_id, timestep in enumerate(tqdm(
        ...
        if apply_copilot:
            ...
            time_embedding = capture_payload["time_embedding"]
            hidden_mean = capture_payload["hidden_mean"]
            condition_embedding = inputs_posi["context"]
...
def main() -> None:
    ...
    print(f"[info] base DiT weights: {dit_path}")
    print(f"[info] LoRA weights:     {lora_path}  (alpha={args.lora_alpha})")
    ...
    # ---- Fuse LoRA into DiT ----
    print(f"[info] fusing LoRA into DiT ...")
    pipe.load_lora(pipe.dit, lora_config=str(lora_path), alpha=args.lora_alpha)
    print(f"[info] LoRA fused successfully")
